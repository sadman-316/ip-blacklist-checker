    ...
    try {
      ...
      try {
        ...
        if (res.ok && data.success && data.user) {
          ...
          if (data.error.includes("Invalid email or password") || data.error.includes("suspended")) {
            throw new Error(data.error);
          }
          console.warn("Server returned login notice, falling back to client credentials/auto-provisioning:", data.error);
        }
      } catch (apiErr: any) {
        if (apiErr.message && (
          apiErr.message.includes("Invalid email or password") ||
          apiErr.message.includes("suspended")
        )) {
          throw apiErr;
        }
        console.warn("Server auth endpoint notice, falling back to local credentials check:", apiErr);
      }
