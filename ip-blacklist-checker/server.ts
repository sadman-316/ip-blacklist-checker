...
const providerResolvers = new Map<string, dns.Resolver[]>();
...
async function resolveDnsWithFallback(lookupDomain: string, providerId: string): Promise<string[]> {
  ...
  if (authResolvers && authResolvers.length > 0) {
    const promises = authResolvers.slice(0, 2).map(resolver => {
    ...
    try {
      const addresses = await Promise.any(promises);
  ...
  // Tier 2: Try explicit custom/private DNS resolvers configured in system settings
  const resolversToTry = systemConfig.dnsResolvers && systemConfig.dnsResolvers.length > 0
    ? systemConfig.dnsResolvers
    : ['1.1.1.1', '8.8.8.8', '9.9.9.9'];

  for (const fallback of resolversToTry) {
    try {
...
async function resolveTxtWithFallback(lookupDomain: string, providerId: string): Promise<string> {
  ...
  if (authResolvers && authResolvers.length > 0) {
    try {
      const records = await new Promise<string[][]>((resolve, reject) => {
        const t = setTimeout(() => reject(new Error('Timeout')), 1500);
        authResolvers[0].resolveTxt(lookupDomain, (err, records) => {
          clearTimeout(t);
          if (err) reject(err);
  ...
  // Try custom/private DNS resolvers configured in system settings
  const resolversToTry = systemConfig.dnsResolvers && systemConfig.dnsResolvers.length > 0
    ? systemConfig.dnsResolvers
    : ['1.1.1.1', '8.8.8.8', '9.9.9.9'];
...
async function checkDNSBL(ip: string, blacklistDomain: string): Promise<{
  ...
  while (retries >= 0) {
    ...
    try {
      ...
      const addresses = await resolveDnsWithFallback(lookupDomain, providerId);
      ...
      // If it is a transient error (SERVFAIL, REFUSED, TIMEOUT) and we have retries left, continue the loop
      if (retries > 0 && (errCode === 'EREFUSED' || errCode === 'ESERVFAIL' || err.message === 'Timeout' || err.message === 'Timeout after 2s')) {
        retries--;
  ...
  return {
    ...
    responseCode: lastError?.code || 'ERROR',
    txt: '',
    responseTime,
    timestamp: nowStr,
    error: true,
    ...
    if (authResolvers && authResolvers.length > 0) {
      try {
        const authA = await new Promise<string[]>((resolve, reject) => {
          const t = setTimeout(() => reject(new Error('Timeout after 2s')), 2000);
          authResolvers[0].resolve4(lookupDomain, (err, addresses) => {
            clearTimeout(t);
            if (err) reject(err);
            else resolve(addresses);
          });
